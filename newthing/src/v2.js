...
export default class Rect {
  ...
  intersects(other) {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      //The amount of distance between the shapes
    }
    if (overlapping.every(a => a <= 0)) {
      //If there is no distance for any axis
      var i = overlapping.indexOf(Math.max(...overlapping));
      //Find the least overlapping vector
      return vectors[i];
      /*
      return vectors[overlapping.reduce(
        (iMin, x, i, arr) => x < arr[iMin] ? i : iMin, Infinity)
      ];
      */
    } else {
      return false;
